...
const { keysArr, handle500Error, handle404Error } = require('../utils/server.js')
const statesArr = require('./states_array.js')

const queries = {
    async returnAllStates(res) {
        await db.all(
            `SELECT * FROM states`,
            [], (err, rows) => {
            if (err) {
    ...
    async returnSingleStateInfo(res, state) {
        await db.all(
    ...
    async returnFieldData(res, query, field) {
        await db.all(
    ...
    async returnCongressData(res, state, legislature, index) {
        await db.all(
            ...
            [state], (err, rows) => {
                if (err) {
                    ...
                    this.returnLegislature(legislature, res, index, err, rows)
                }
            }
        )
    },
    ...
    async parseQuery(res, query, field, index) {
        if (!field && statesArr.includes(query)) {
            this.returnSingleStateInfo(res, query)
            ...
            if (!index && keysArr.includes(field)) {
                this.returnFieldData(res, query, field)
            } else if (index && keysArr.includes(field)) {
                if (field === "senators" || field === "house_delegation") {
                    this.returnCongressData(res, query, field, index)
                } else {
                    return handle404Error(res)
                }
            }
